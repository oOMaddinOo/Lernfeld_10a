...
namespace WaterDropTests.UnitTests
{
	...
	public class KloServiceTests : IDisposable
	{
		...
		[Fact]
		public async Task GetKlosByElementIds_WithMultipleReviewsPerElement_ShouldReturnLatestReviewPerElement()
		{
			// Arrange
			var elementId = 700006L;
			var older = CreateTestKloModel("Alt", elementId);
			older.CreatedAt = new DateTime(2026, 3, 1, 9, 0, 0, DateTimeKind.Utc);

			var newer = CreateTestKloModel("Neu", elementId);
			newer.CreatedAt = new DateTime(2026, 3, 1, 10, 0, 0, DateTimeKind.Utc);

			var other = CreateTestKloModel("Anderer Ort", 700007L);
			other.CreatedAt = new DateTime(2026, 3, 1, 11, 0, 0, DateTimeKind.Utc);

			await _service.AddKloCommentToData(older);
			await _service.AddKloCommentToData(newer);
			await _service.AddKloCommentToData(other);

			// Act
			var result = await _service.GetKlosByElementIds(new[] { elementId, 700007L });

			// Assert
			Assert.Equal(2, result.Count);
			Assert.Equal("Neu", result.Single(r => r.ElementId == elementId).Comment);
			Assert.Equal("Anderer Ort", result.Single(r => r.ElementId == 700007L).Comment);
		}
